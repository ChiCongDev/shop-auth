...
<div id="danh-sach-don-hang-doi-tac" class="mx-auto max-w-7xl px-4 pb-8 pt-6 sm:px-6 sm:pt-8 lg:px-8">
    <div class="mb-6 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
            <p class="text-xs font-bold uppercase tracking-wide" style="color:#d4af37">Theo doi ban hang</p>
            <h1 class="mt-1 text-2xl font-bold text-gray-900">Don hang</h1>
            <p class="mt-1 text-sm text-gray-500">Danh sach don thuong duoc lay tu he thong quan ly noi bo.</p>
        </div>
    ...
    <div class="mb-6 grid grid-cols-2 gap-3 md:grid-cols-3 xl:grid-cols-6">
        @foreach([
            ['ma' => 'cho_xu_ly', 'ten' => 'Cho xu ly', 'lop' => 'bg-yellow-100 text-yellow-700'],
            ['ma' => 'xuat_kho', 'ten' => 'Xuat kho', 'lop' => 'bg-blue-100 text-blue-700'],
            ['ma' => 'dong_goi', 'ten' => 'Dong goi', 'lop' => 'bg-indigo-100 text-indigo-700'],
            ['ma' => 'van_chuyen', 'ten' => 'Dang giao', 'lop' => 'bg-purple-100 text-purple-700'],
            ['ma' => 'hoan_thanh', 'ten' => 'Hoan thanh', 'lop' => 'bg-green-100 text-green-700'],
            ['ma' => 'huy', 'ten' => 'Da huy', 'lop' => 'bg-red-100 text-red-700'],
        ] as $item)
    ...
    <div class="overflow-hidden rounded-lg border border-gray-200 bg-white shadow-sm">
        ...
        <div class="border-b border-gray-100 p-4">
            <div class="flex flex-col gap-3 xl:flex-row">
                <div class="relative flex-1">
                    <input id="input-search-don-hang" class="w-full rounded-lg border border-gray-200 py-3 pl-10 pr-4 text-sm focus:outline-none focus:ring-2 focus:ring-yellow-400" placeholder="Tim theo ma don, ten hoac SDT khach hang">
                ...
                <select id="filter-ngay-tao" class="rounded-lg border border-gray-200 bg-white px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-yellow-400 xl:w-44">
                    ...
                    <option value="yesterday">Hom qua</option>
                    <option value="7days">7 ngay qua</option>
                    <option value="this_week">Tuan nay</option>
                    <option value="30days">30 ngay qua</option>
                ...
                <select id="filter-trang-thai" class="rounded-lg border border-gray-200 bg-white px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-yellow-400 xl:w-44">
                    ...
                    <option value="xuat_kho">Xuat kho</option>
                    <option value="dong_goi">Dong goi</option>
                    <option value="van_chuyen">Dang giao</option>
                    <option value="tu_van_chuyen">Tu van chuyen</option>
                    <option value="hoan_thanh">Hoan thanh</option>
                    <option value="huy">Da huy</option>
                </select>
                <input id="filter-khach-hang" class="rounded-lg border border-gray-200 px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-yellow-400 xl:w-44" placeholder="Ma/ten khach">
                <input id="filter-san-pham" class="rounded-lg border border-gray-200 px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-yellow-400 xl:w-44" placeholder="SKU/ten SP">
            </div>
            <div id="custom-date-range" class="mt-3 hidden flex-col gap-3 sm:flex-row sm:items-center">
                ...
                <span class="text-sm text-gray-400">den</span>
                ...
                <button id="btn-xoa-bo-loc" type="button" class="rounded-lg px-4 py-3 text-sm font-semibold text-red-600 hover:bg-red-50">Xoa loc</button>
            </div>
        </div>
        ...
        <div class="overflow-x-auto">
            <table class="w-full min-w-[980px] text-sm">
                <thead class="bg-gray-50 text-xs uppercase tracking-wide text-gray-500">
                    <tr>
                        <th class="px-4 py-3 text-left">Ma don hang</th>
                        <th class="px-4 py-3 text-left">Ngay tao</th>
                        <th class="px-4 py-3 text-left">Khach hang</th>
                        <th class="px-4 py-3 text-left">Nhan vien</th>
                        <th class="px-4 py-3 text-left">Trang thai</th>
                        <th class="px-4 py-3 text-right">Khach phai tra</th>
        ...
        <div id="loading-don-hang" class="py-12 text-center text-sm text-gray-500">Dang tai danh sach don hang...</div>
        <div id="empty-don-hang" class="hidden py-12 text-center text-sm text-gray-500">Khong co don hang phu hop.</div>
...
function badge(status) {
    const cls = {
        ...
        dong_goi: 'bg-indigo-100 text-indigo-800',
...
function renderDanhSach(items) {
    ...
    tbody.innerHTML = items.map(item => {
        ...
        return `<tr class="transition hover:bg-gray-50">
            ...
            <td class="px-4 py-4">
                <div class="font-semibold text-gray-900">${escapeHtml(item.khach_hang?.ten || 'Khach le')}</div>
...
function renderPagination(p) {
    ...
    document.getElementById('pagination-info').textContent = `Hien thi ${from}-${to} / ${p.total} don`;

    const pages = new Set([1, p.last_page, p.current_page - 1, p.current_page, p.current_page + 1]);
    let last = 0;
    const buttons = [pageButton('Truoc', Math.max(1, p.current_page - 1), { disabled: p.current_page <= 1 })];
    [...pages]
...
async function taiDanhSach(page = 1) {
    ...
    try {
        ...
        showToast('Loi', error.message || 'Khong the tai danh sach don hang');
    } finally {
